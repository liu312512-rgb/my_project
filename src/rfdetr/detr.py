...
from rfdetr.sci_enhance import Finetunemodel as SCIFinetunemodel
...
class RFDETR:
    ...
    def __init__(self, **kwargs):
        # Pop SCI-related kwargs before they reach ModelConfig
        self._enhance_low_light = kwargs.pop("enhance_low_light", False)
        _sci_weights = kwargs.pop("sci_weights", None)
        ...
        # --- Low-light enhancement (SCI++) ---
        self._sci_model: torch.nn.Module | None = None
        if self._enhance_low_light:
            if _sci_weights is None:
                _sci_weights = os.path.join(os.path.dirname(__file__), "sci_weights", "weights_1_3500.pt")
            if not os.path.isfile(_sci_weights):
                raise FileNotFoundError(f"SCI weights not found at {_sci_weights}")
            self._sci_model = SCIFinetunemodel(_sci_weights)
            self._sci_model.eval()
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        # ---- Low-light enhancement via SCI++ ----
        # Runs on [0,1] tensors *before* ImageNet normalization so the
        # enhancer sees natural pixel values.  Resize happens after
        # enhancement so SCI operates at the original resolution.
        if self._sci_model is not None:
            self._sci_model = self._sci_model.to(self.model.device)
            with torch.inference_mode():
                processed_images = [
                    self._sci_model(img.unsqueeze(0))[0].squeeze(0) for img in processed_images
                ]
